		...
		?>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    <td><?= $application->hasValue('staff') ? $this->Html->link($application->staff->name, ['controller' => 'Staffs', 'action' => 'view', $application->staff->id]) : '' ?></td>
                    <td><?= $application->hasValue('department') ? $this->Html->link($application->department->name, ['controller' => 'Departments', 'action' => 'view', $application->department->id]) : '' ?></td>
                    <td><?= $application->hasValue('type') ? $this->Html->link($application->type->name, ['controller' => 'Types', 'action' => 'view', $application->type->id]) : '' ?></td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="fa-solid fa-eye"></i>'), ['action' => 'view', $application->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body ">
		<div class="card-title mb-2">SEARCH</div>
			<div class="tricolor_line mb-5"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Applications','action' => 'index']]); ?>
				<fieldset>
					...
					<div class="mb-1"><?php echo $this->Form->control('department_id', ['options' => $departments, 'class'=>'form-select','empty'=>'Select Department', 'required' => false]); ?></div>
				</fieldset>
		...
		<?= $this->Html->link('<i class="fa-solid fa-plus"></i> New Application', ['action' => 'add'], ['class' => 'btn btn-primary btn-sm', 'escapeTitle' => false]); ?>
</div>
	</div>
</div>
</div>
